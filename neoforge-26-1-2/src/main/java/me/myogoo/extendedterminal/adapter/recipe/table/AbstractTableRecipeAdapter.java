...
abstract class AbstractTableRecipeAdapter implements MyoTableRecipe {
    ...
    AbstractTableRecipeAdapter(Recipe<?> recipe, int tier, ResourceKey<Recipe<?>> recipeId) {
        ...
        for (int i = 0; i < ingredients.size(); i++) {
            ...
            if (ingredient != null) {
                var stack = ingredientPriorities.entrySet()
                        ...
                        .orElseGet(() -> ingredient.getValues().stream()
                                .findFirst()
    ...
    protected List<Optional<Ingredient>> recipeIngredients() {
        return MyoTableRecipe.positionedIngredients(recipe);
    }
    ...
    public ItemStack assemble(MyoTableInput input, Level level, MyoRecipeType recipeType) {
        ...
        if (annotation == ExtendedCrafting.class) {
            if (this.recipe instanceof ITableRecipe tableRecipe) {
                return MagicRecipe.assemble(tableRecipe, input);
            }
        }
        ...
        if (annotation == VANILLA.class) {
            return MagicRecipe.assemble((CraftingRecipe) this.recipe, input);
        }
    ...
    private static class MagicRecipe {
        public static ItemStack assemble(ITableRecipe recipe, MyoTableInput input) {
            return recipe.assemble(TableCraftingInput.of(input.width(), input.height(), input.items(), input.tier()));
        }

        public static ItemStack assemble(ITierCraftingRecipe recipe, MyoTableInput input) {
            return recipe.assemble(TierInput.of(input.width(), input.height(), input.items(), input.tier()));
        }

        public static ItemStack assemble(CraftingRecipe recipe, MyoTableInput input) {
            return recipe.assemble(input.cast());
        }
