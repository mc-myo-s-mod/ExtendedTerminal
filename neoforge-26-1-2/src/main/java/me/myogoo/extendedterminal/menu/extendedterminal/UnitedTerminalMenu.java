...
public class UnitedTerminalMenu extends TableTerminalBaseMenu<Recipe<RecipeInput>> {
    public static final ClientActionKey<Boolean> ACTION_REMEMBER_RECIPE_TYPE = new ClientActionKey<>("rememberRecipeType");
    private static final ClientActionKey<Void> ACTION_SELECT_NEXT_RECIPE_TYPE = new ClientActionKey<>("selectNextRecipeType");
    private static final ClientActionKey<Void> ACTION_SELECT_PREVIOUS_RECIPE_TYPE = new ClientActionKey<>("selectPreviousRecipeType");
    ...
    public UnitedTerminalMenu(MenuType<?> menuType, int id, Inventory ip, IUnitedTerminalHost host) {
        ...
        registerClientAction(ACTION_REMEMBER_RECIPE_TYPE, ByteBufCodecs.BOOL, this::setRememberRecipeType);
    ...
    private void selectRecipeTypeOffset(int offset, ClientActionKey<Void> clientAction) {
        if (isClientSide()) {
    ...
    private static MyoTableRecipe findExtendedCraftingRecipe(Level level, MyoTableInput input) {
        var recipe = RecipeAccess.getRecipeFor(level, ModRecipeTypes.TABLE.get(), TableCraftingInput.of(input.width(), input.height(), input.items(), input.tier()));
        return recipe != null ? MyoTableRecipe.of(recipe.value(), recipe.id()) : null;
    }

    @Nullable
    private static MyoTableRecipe findVanillaRecipe(Level level, MyoTableInput input) {
        var recipe = RecipeAccess.getRecipeFor(level, RecipeType.CRAFTING, input.cast());
        return recipe != null ? MyoTableRecipe.of(recipe.value(), recipe.id()) : null;
    }

    @Nullable
    private static MyoTableRecipe findReAvaritiaRecipe(Level level, MyoTableInput input) {
        var recipe = RecipeAccess.getRecipeFor(level, committee.nova.mods.avaritia.init.registry.ModRecipeTypes.CRAFTING_TABLE_RECIPE.get(), TierInput.of(input.width(), input.height(), input.items(), input.tier()));
        return recipe != null ? MyoTableRecipe.of(recipe.value(), recipe.id()) : null;
