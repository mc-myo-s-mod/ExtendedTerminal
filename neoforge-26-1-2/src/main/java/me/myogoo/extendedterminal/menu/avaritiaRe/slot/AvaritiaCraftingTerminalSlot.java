...
public class AvaritiaCraftingTerminalSlot extends ETCraftingBaseSlot<ITierCraftingRecipe, TierInput> {
    public AvaritiaCraftingTerminalSlot(Player player, IActionSource mySrc, IEnergySource energySrc, MEStorage storage, InternalInventory cMatrix, InternalInventory secondMatrix, ICraftingGridMenu ccp, ETMenuType menuType) {
    ...
    protected RecipeHolder<ITierCraftingRecipe> findRecipe(TierInput ic, Level level) {
        ...
        return RecipeAccess.getRecipeFor(level, ModRecipeTypes.CRAFTING_TABLE_RECIPE.get(), ic);
    }

    @Override
    protected NonNullList<ItemStack> getETRemainingItems(TierInput ic, Level level) {
        if (this.menu instanceof AvaritiaTerminalBaseMenu terminalMenu) {
            var recipe = terminalMenu.getCurrentRecipe();
        ...
        var recipe = RecipeAccess.getRecipeFor(level, ModRecipeTypes.CRAFTING_TABLE_RECIPE.get(), ic);
        if (recipe != null) {
            return recipe.value().getRemainingItems(ic);
        }
        return NonNullList.withSize(menuType.getGridSize(), ItemStack.EMPTY);
    }
    ...
    protected void makeItem(Player player, ItemStack stack) {
        ...
        TierInput positioned = TierInput.of(
    ...
    protected ItemStack craftItem(Player p, MEStorage inv, KeyCounter all) {
        ...
        if (!level.isClientSide()) {
            ...
            var recipeInput = TierInput.of(menuType.getGridSideLength(), menuType.getGridSideLength(), ic, menuType.getTier());
