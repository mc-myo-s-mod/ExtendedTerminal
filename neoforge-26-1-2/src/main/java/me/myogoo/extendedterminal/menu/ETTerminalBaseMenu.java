...
public abstract class ETTerminalBaseMenu<R extends Recipe<?>> extends MEStorageMenu implements ICraftingGridMenu {
    ...
    private static final ClientActionKey<Identifier> ACTION_CLEAR_TO_PLAYER = new ClientActionKey<>("clearToPlayer");
    ...
    public ETTerminalBaseMenu(MenuType<?> menuType, int id, Inventory ip, ITerminalHost host, ETMenuType etMenuType, IETTerminalConfig config) {
        ...
        this.config = config;
        registerClientAction(ACTION_CLEAR_TO_PLAYER, Identifier.STREAM_CODEC, this::clearToPlayerInventory);
    }
    ...
    public CraftingTermMenu.MissingIngredientSlots findMissingIngredients(Map<Integer, Ingredient> ingredients) {
        ...
        var reservedPlayerItems = new int[playerInventory.getNonEquipmentItems().size()];
        ...
        for (var entry : ingredients.entrySet()) {
            ...
            for (int i = 0; i < reservedPlayerItems.length; i++) {
                ...
                if (clientRepo != null) {
                    for (var stack : clientRepo.getAllEntries()) {
                        if (stack.isCraftable()
                                && stack.getWhat() instanceof AEItemKey itemKey
    ...
    public void clearToPlayerInventory(Identifier invId) {
        if (isClientSide()) {
