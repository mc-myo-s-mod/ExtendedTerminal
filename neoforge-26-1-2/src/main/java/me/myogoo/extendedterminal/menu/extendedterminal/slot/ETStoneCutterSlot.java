...
public class ETStoneCutterSlot extends ETCraftingBaseSlot<StonecutterRecipe, SingleRecipeInput> {
    ...
    protected RecipeHolder<StonecutterRecipe> findRecipe(SingleRecipeInput ic, Level level) {
        ...
        if (terminalMenu.getStoneCutterRecipeId() == null) {
            return null;
        }
        var recipeKey = ResourceKey.create(Registries.RECIPE, terminalMenu.getStoneCutterRecipeId());
        return RecipeAccess.byKey(level, RecipeType.STONECUTTING, recipeKey);
    }
    ...
    protected NonNullList<ItemStack> getETRemainingItems(SingleRecipeInput ic, Level level) {
        if(this.menu instanceof ETTerminalMenu terminalMenu) {
            ...
            if(recipeId != null) {
                var recipe = RecipeAccess.byKey(level, RecipeType.STONECUTTING, ResourceKey.create(Registries.RECIPE, recipeId));
