...
public final class AE2HelpersUpgradeRegistration {
    ...
    private static Optional<Item> findImportExportCard(String path) {
        var modernId = Identifier.fromNamespaceAndPath("ae2importexportcard", path);
        var legacyId = Identifier.fromNamespaceAndPath("ae2insertexportcard", path);
    ...
    private static Optional<Item> findResultImportCard() {
        var canonicalId = Identifier.fromNamespaceAndPath("ae2helpers", "result_import_card");
        return BuiltInRegistries.ITEM.getOptional(canonicalId);
    }
}
