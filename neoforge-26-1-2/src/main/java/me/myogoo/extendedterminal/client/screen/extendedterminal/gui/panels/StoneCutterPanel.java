...
public class StoneCutterPanel extends ETTerminalModePanel {
    ...
    private void drawRecipes(GuiGraphicsExtractor guiGraphics, Rect2i bounds, Point mouse) {
        ...
        for (int i = startIndex; i < endIndex && i < recipes.size(); ++i) {
            ...
            boolean selected = selectedRecipe != null && selectedRecipe.equals(recipe.id().identifier());
            ...
            ItemStack resultItem = recipe.value().assemble(new SingleRecipeInput(menu.getStoneCutterInventory().getStackInSlot(0)));
    ...
    public boolean onMouseDown(Point mousePos, int button) {
        ...
        if (recipe != null) {
            menu.setStoneCutterRecipeId(recipe.id().identifier());
    ...
    public Tooltip getTooltip(int mouseX, int mouseY) {
        ...
        if (recipe != null) {
            var result = recipe.value().assemble(new SingleRecipeInput(menu.getStoneCutterInventory().getStackInSlot(0)));
            var lines = screen.getTooltipFromContainerItem(result);
