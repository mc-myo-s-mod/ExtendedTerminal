package me.myogoo.extendedterminal.menu.avaritiaNeo.slot;
...
public class AvaritiaNeoCraftingTerminalSlot extends ETCraftingBaseSlot<RecipeExtremeCrafting, CraftingInput> {
    public AvaritiaNeoCraftingTerminalSlot(Player player, IActionSource mySrc, IEnergySource energySrc, MEStorage storage, InternalInventory cMatrix, InternalInventory secondMatrix, ICraftingGridMenu ccp) {
        super(player, mySrc, energySrc, storage, cMatrix, secondMatrix, ccp, ETMenuType.NEO_EXTREME_TERMINAL);
    }
    ...
    protected NonNullList<ItemStack> getETRemainingItems(CraftingInput ic, Level level) {
        ...
        return this.getRemainingItems(ic, level);
    }
    ...
    protected ItemStack craftItem(Player p, MEStorage inv, KeyCounter all) {
        ...
        if (!level.isClientSide()) {
            ...
            var recipeInput = CraftingInput.of(menuType.getGridSideLength(), menuType.getGridSideLength(), ic);
