...
public class AvaritiaTerminalBaseMenu extends ETTerminalBaseMenu<BaseTableCraftingRecipe> {
    ...
    protected void updateCurrentRecipeAndOutput(boolean forceUpdate) {
        ...
        this.currentRecipe = level.getRecipeManager().getRecipeFor(ModRecipeTypes.CRAFTING_TABLE_RECIPE.get(), recipeTestContainer, level)
                .orElse(null);
    ...
    public void doAction(ServerPlayer player, InventoryAction action, int slot, long id) {
        ...
        if(s instanceof AvaritiaCraftingTerminalSlot craftingSlot) {
            switch (action) {
                case CRAFT_SHIFT:
                case CRAFT_ALL:
                case CRAFT_ITEM:
