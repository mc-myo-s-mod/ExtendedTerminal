...
public class ECTableRecipe extends BasicEmiRecipe {
    private static final int BASE_IN_X = 62;
    private final ITableRecipe recipe;
    private final ClientLevel level;

    public ECTableRecipe(EmiRecipeCategory category, ITableRecipe recipe) {
        super(category, recipe.getId(), calcInvX(recipe.getTier()), calcInvY(recipe.getTier()));
        this.recipe = recipe;
    ...
    public void addWidgets(WidgetHolder widgets) {
        for (int i = 0; i < this.getGridSize(); i++) {
            EmiIngredient ingredient = i < this.inputs.size()
                    ? this.inputs.get(i)
            ...
            widgets.addSlot(ingredient, nx, ny).drawBack(false);
        }

        int outputX = calcInvX(this.recipe.getTier());
        int outputY = (this.getTierColNRow() / 2) * 18;

        if (this.recipe instanceof ShapelessTableRecipe) {
            widgets.addTexture(EmiTexture.SHAPELESS, outputX - 25, outputY - 18);
        }
        ...
        widgets.addTexture(EmiTexture.LARGE_SLOT, outputX - 30, outputY - 4).tooltip(List.of(tooltip));
        widgets.addSlot(EmiStack.of(recipe.getResultItem(level.registryAccess())), outputX - 30, outputY - 4)
                .large(true)
                .recipeContext(this)
                .drawBack(false);
    }
    ...
    private int getTierColNRow() {
        return this.recipe.getTier() * 2 + 1;
    }

    private int getGridSize() {
        int side = this.getTierColNRow();
        return side * side;
    }

    private void addIngredients(ITableRecipe recipe) {
        ...
        if (adapter instanceof IShapedTableRecipeAdapter shapedAdapter) {
            ...
            this.inputs = new ArrayList<>(Collections.nCopies(this.getGridSize(), EmiIngredient.of(Ingredient.EMPTY)));
