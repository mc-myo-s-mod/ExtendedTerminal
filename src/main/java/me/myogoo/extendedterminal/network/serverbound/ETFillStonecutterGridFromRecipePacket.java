...
public class ETFillStonecutterGridFromRecipePacket extends FillRecipeBasePacket implements IMyotusPacket {
    ...
    public ETFillStonecutterGridFromRecipePacket(FriendlyByteBuf stream) {
        ResourceLocation decodedRecipeId = null;
        if (stream.readBoolean()) {
            decodedRecipeId = stream.readResourceLocation();
        }
        this.recipeId = decodedRecipeId;
        this.ingredientTemplate = stream.readItem();
        this.craftMissing = stream.readBoolean();
    }

    public ETFillStonecutterGridFromRecipePacket(@Nullable ResourceLocation recipeId, ItemStack ingredientTemplate,
            boolean craftMissing) {
        this.recipeId = recipeId;
        this.ingredientTemplate = ingredientTemplate.copy();
    ...
    public void write(FriendlyByteBuf stream) {
        ...
        stream.writeItem(this.ingredientTemplate);
        stream.writeBoolean(this.craftMissing);
    }

    public static void handle(ETFillStonecutterGridFromRecipePacket packet, MyoPacketContext context) {
    ...
    private void handleOnServer(ServerPlayer player) {
        ...
        for (int x = 0; x < craftMatrix.size(); x++) {
            ...
            var ingredient = ingredients.isEmpty() ? Ingredient.EMPTY : ingredients.get(0);

            if (!currentItem.isEmpty()) {
                if (ingredient.test(currentItem)) {
                    if (this.recipeId != null && !this.recipeId.equals(cct.getStoneCutterRecipeId())) {
    ...
    protected NonNullList<Ingredient> getDesiredIngredients(Player player) {
        if (this.recipeId != null) {
            ...
            if (optionalRecipe.isPresent() && optionalRecipe.get() instanceof StonecutterRecipe recipe) {
                return recipe.getIngredients();
            }
        }

        var ingredients = NonNullList.withSize(1, Ingredient.EMPTY);
        if (!this.ingredientTemplate.isEmpty()) {
            ingredients.set(0, Ingredient.of(this.ingredientTemplate));
        }
        return ingredients;
    }
}
